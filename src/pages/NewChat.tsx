        ...
        <div className='flex gap-2'>
            {
                suggestions.map((suggestion, i) => (
                    <div className='group relative p-[2px] overflow-hidden rounded-xl'>
                        <div 
                            key={`prompt_${i}`}
                            className={`bg-[#FAFAFA] border border-[#7F7F7F20] rounded-lg w-50 h-40 p-3 text-[14px] flex flex-col justify-between ${!disabled && 'cursor-pointer'}`}
                        ...
                        <div
                            ...
                            style={{ background: "conic-gradient(transparent, #C58EFF80, #709DFF80, transparent)", zIndex: -1 }}
                        ></div>
                    </div>
                ))
            }
...
const NewChat = () => {
    ...
    return (
        <div className={`${className} gap-10 justify-center`}>
            <Carly cursorPos={cursorPos}/>
            <div className={`${className} gap-8`}>
                <h1 className='text-4xl font-medium'>What can I help you with?</h1>
                <div className={`${className} items-start gap-4`}>
                    <SuggestedPrompts 
                        disabled={isLoading}
